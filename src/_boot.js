...
process.on("uncaughtException", e => {
    signale.fatal(e);
    dialog.showErrorBox("BD Nuclear Networking crashed", e.message || "Cannot retrieve error message.");
    if (tty) {
        tty.close();
    }
    if (extraTtys) {
...
signale.start(`Starting BD Nuclear Networking v${app.getVersion()}`);
...
app.on('ready', async () => {
    ...
    Object.assign(cleanEnv, {
        TERM: "xterm-256color",
        COLORTERM: "truecolor",
        TERM_PROGRAM: "BD Nuclear Networking",
